...
def pgSketchyCoreTuckerPermute(A,r,start,step,stop):
    ...
    while np.sum(m) < stop:
        ...
        params[0] = entropy(np.var(Q0,axis=0))
        params[1] = entropy(np.var(Q1,axis=0))
        params[2] = entropy(np.var(Q2,axis=0))
...
def lc(N,A,A0,A1,A2,r,logPS_0,logPS_1,logPS_2):
    err_log = []
...
'''
class Node:
    def __init__(self, *, x: int, y: int, h: int, w: int, **kwargs):
        self.x = x
        self.y = y
        self.w = w
        self.h = h
    ...
    def __call__(self, *, x: float, y: float):
        # Check that x and y are between 0 and 1
        assert 0 <= x <= 1 and 0 <= y <= 1, "x and y must be between 0 and 1"

        w = int(x * self.w)
        h = int(y * self.h)

        # Top left
        tl = Node(x=self.x, y=self.y, h=h, w=w)
        tr = Node(x=self.x + w, y=self.y, h=h, w=self.w - w)
        bl = Node(x=self.x, y=self.y + h, h=self.h - h, w=w)
        br = Node(x=self.x + w, y=self.y + h, h=self.h - h, w=self.w - w)

        self.children = [tl, tr, bl, br]
        return self.children

    def __repr__(self) -> str:
        return f"Node(x={self.x}, y={self.y}, h={self.h}, w={self.w})"
